 ...
 */
export function detectZone(position3D: Position3D): string {
  const { x, y, z } = position3D;

  const sections = roomData.sections as RoomSection[];
 ...
 * 특정 방의 중심 좌표를 반환합니다.
 */
export function getZoneCenter(zoneName: string): Position3D | null {
  const sections = roomData.sections as RoomSection[];
 ...
 * Room.json 정보 요약 출력
 */
export function printRoomInfo(): void {
  const sections = roomData.sections as RoomSection[];
  console.log('\n📍 Room.json 정보:');
