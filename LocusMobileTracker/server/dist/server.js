...
wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (data) => {
        try {
            ...
            switch (raw.type) {
                case 'arkit_location':
                    handleARKitLocation(clientId, raw);
                    break;
                case 'location':
                    // 예전 GPS 클라이언트가 있다면 여기서 처리
                    handleLegacyLocation(clientId, raw);
                    break;
                case 'identify':
            ...
            const err = error;
            console.error(`[오류] 메시지 파싱 실패 (클라이언트 #${clientId}):`, err.message);
        }
    });
    // 연결 종료 처리
    ws.on('close', () => {
 ...
 * ARKit 3D 위치 처리
 */
function handleARKitLocation(clientId, message) {
    const { position3D, accuracy, timestamp, } = message.data;
    // Zone 자동 감지
    const zone = detectZone(position3D);
    console.log(`  [ARKit] 3D 좌표: (${position3D.x.toFixed(3)}, ${position3D.y.toFixed(3)}, ${position3D.z.toFixed(3)})`);
 ...
 */
function handleLegacyLocation(clientId, locationData) {
    const { latitude, longitude, accuracy, timestamp, altitude, heading, speed, } = locationData;
